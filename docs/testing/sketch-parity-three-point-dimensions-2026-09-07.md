...
Gallery Undo diagnostic `/tmp/os3d-gallery-undo-20260907.xcresult` failed post-Redo badge expectation, after successful Undo badge removal. Redo need not restore selection; test revised to inspect usable profile: absent afterUndo, extrudable afterRedo. Rerun `/tmp/os3d-gallery-undo-profile-20260907.xcresult` active. Do not count initial diagnostic as clean pass.

Gallery profile rerun passed1/1: screenshots inspected, newly drawn lower rectangle absent afterUndo and restored/extrudable afterRedo. Existing upper rectangle retained. This supports selection-only explanation of first failure. Evidence `gallery-undo-automated/`, explicitly automated-only. Live Peekaboo toolbar behavior remains unresolved; no Undo implementation change.
