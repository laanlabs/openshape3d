...
### Text on a top face reads sideways (found 2026-09-18)

- A sketch started by picking a horizontal top face (normal +Y) gets
  xAxis +Z, yAxis +X, whatever built the body; the ground plane is +X, -Z.
  The Text tool lays glyphs along the sketch +x, so a name written on a
  tag's top face runs along world Z and reads upside-down in
  `view.isometric`. Probe: `scripts/youtube_series/probe_text.py`. The
  keychain tutorial works around it (tag laid along Z, finished part turned
  135°); the fix belongs in the face → sketch-plane basis.
