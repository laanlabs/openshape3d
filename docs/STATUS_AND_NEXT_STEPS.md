...
- **Bug:** a sketch started on a picked face took the face's mesh basis
  (`FaceTopology.PlanarFace.basisX`, its first boundary edge). On a box's
  top face that is +Z, so the sketch plane was xAxis +Z / yAxis +X and Text
  written there ran across the view (upside-down in `view.isometric`,
  sideways in `view.top`); a front face got xAxis +Y (text ran upward).
  Found recording the keychain tutorial (2026-09-18).
- **Fix:** `SketchPlane.readable(origin:normal:)` (`SketchPlanes.swift`)
  lays a face sketch out like the world planes: within 1° of horizontal it
  takes the ground's axes (x +X; y -Z facing up, +Z facing down), otherwise
  y is world up projected into the face and x completes the frame (a +Z
  face gets `worldXY`'s axes, +X `worldYZ`'s). Used by the plane picker
  (`worldFacePlane`, also Section View), select-a-face-then-Sketch, and
  Offset Plane from a face. The face selection's own plane keeps the mesh
  basis, because `moveFace` deltas are stored in it; existing sketches keep
  their stored planes (`PlaneRef` has no face-anchored source, so nothing
  re-derives them).
- **Verified:** `FaceSketchPlaneTests` (7: up/down/vertical/sloped faces, a
  real box top face, Text on a top face runs along +X); full unit target
  1732 tests, 0 failures; live app over the bridge — picking a top face now
  gives x +X / y -Z, a front face x +X / y +Y (were +Z/+X). On the
  `os3d-video` simulator `PlanesUITests.testSketchOnFaceThenExtrudeNewBody`
  and `testWorldTileTapStartsSketchOnThatPlane` fail at the same lines with
  and without this change (baseline run on unmodified main).

## Mission log — 2026-09-18, ten project tutorials for YouTube

- **Ten narrated videos** in `marketing/youtube/` (gitignored), the classic
  CAD-tutorial projects: `openshape3d-{coffee-mug, chess-pawn-rook,
  lego-brick, name-keychain, twisted-vase, bolt-and-nut-threads, spur-gear,
  fidget-spinner, ice-cube-tray, print-in-place-hinged-box}-tutorial.mp4`,
  1:55–2:57 each at 1920×1080, about −16 LUFS, each with `-metadata.md` (title,
  description with chapter times, tags) and a 1280×720 `-thumbnail.png`.
- **One builder per video** in `scripts/youtube_series/projects.py`: a
  generator that yields chapter ids. `project_tutorial.py <name> --dry`
  builds it on the running app and runs `/v1/check`; the take runs the same
  code, so the recorded geometry is the checked geometry. All ten build
  valid B-reps. Fit checks by intersection (the kernel refuses an empty
  result): the hinged box's lid never touches the box, the two gears never
  touch, and the nut screwed onto the bolt in thread phase clears it while
  half a pitch off they collide (56 mm³).
- **Real touches** in every take: Start a Blank Design, tap a sketch region
  and type the height on the keypad, the Text tool (keychain: plane pick,
  placement, typing, Add), Material presets (checked against `/v1/state`
  selection before applying), Export ▸ STL/3MF through the save panel
  (new `save_sheet` action answers "Replace Existing Items?"), History.
  Each has a bridge fallback.
- **Gotchas** (README): `view.fit` right after a `view.<orientation>`
  cancels the turn; a touch-built body stays selected until a tap on empty
  grid; the save panel's Save only exists after a few seconds; a failed take
  must stop `recordVideo` (now in `run_take`'s `finally`); the mug handle's
  sweep fails with a 48-segment path and builds with 30; the bridge has no
  text op, so letter seeds come from rasterising the glyph loops.
- **Narration pronunciation guide** (2026-09-19): `pronunciation.py` respells
  jargon in the text sent to the voice only (CAD /kæd/ one syllable, fillet
  FILL-it, involute IN-vuh-loot, LEGO LEG-oh, …); `common.synthesize` applies
  it. The ten videos were re-voiced with it (recordings reused).
- The keychain's tag-along-Z + 135° turn works around the face-sketch basis
  bug fixed by the 2026-09-19 entry above; it can go on the next re-record.
