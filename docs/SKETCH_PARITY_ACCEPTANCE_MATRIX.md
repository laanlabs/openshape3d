...
| QA-24 | Multi-selection | Core — partial, not passed | Typed planar-face/body/profile chooser on ab6fb2d passed 26/26; changed live face/profile dispatch, Cancel and gallery recovery of three boxes plus circle verified. Body-name/Rename correction b5bff74 final46/46 plus paired history/reopen; illustrated1271unique/1274placements/master38 verified. Connected rectangle and later constraint additive routes retained; native Profile identity confirmed by accessibility selection; broader curved-face/edge selection remains open. 2026-09-13: model-mode Move/Rotate of selected sketch geometry implemented from a native probe (4347508; gizmo on model-mode sketch selections, whole sketch as a rigid frame with dependents rebuilt, subsets in-plane, off-plane subsets split into a new sketch as native does — observed later the same day, implemented at 97f320a); clone paired live; evidence local. Curved-face/edge breadth probed natively on a cylinder and matched at 637a24d (curved faces in Select Through; edge tap arms Chamfer/Fillet). Open: native's edge-only selection state and circular-edge radius readout. |
...
| QA-29 | Badge layout | Core — partial live pass | Portrait and landscape edge keypad usable; resize alignment fixed; right-palette badge and compact keypad overlaps fixed 2026-09-13 (273f0c4); the three logged DimensionUITests failures were grid snapping, tests green (9e4b2cc). 2026-09-13 dense/reposition/zoom paired: dense labels read in full (clone `%g` cap fixed, f27e277), native drags a linear label's leader (measured: selection-lived; driving: saved, undoable) and the clone now matches (97f320a), labels keep screen size across zoom (native unclamped, clone clamped to canvas). Circle label re-checked (measured: radial drag, reverts on reselect). Overlaps paired: no per-entity labels for a multi-selection on either side; parallel-distance readout matched (93df1d3). Remaining: clone zoom on a device. |
...
| QA-33 | Invalid numeric input | Core — PASSED (September 13 finite recipe; ceiling an accepted scope exception) | Paired zero/negative, empty/division/syntax, correction, click-away/Escape and history sampled Sept9; selected polygon count2 refusal/3.5 recovery live; invalid-input/history gate19/19; diagnosis illustrated803/master38 published. Native side-count bound measured to completion 2026-09-13 (1,000/2,000/3,000 sides ≈ 10/47/119 s, no refusal, quadratic; 10,001 ≈ 22 min). Clone 10,000 ceiling kept on keypad and exec paths (d2ff046, 84/84) — accepted scope exception (Jason, 2026-09-13); appendix doc published. See testing/sketch-parity-polygon-upper-bound-2026-09-13.md. |
...
| QA-40 | Keypad transitions | Core — partial, not passed | Retained click-away/tool/toggle/Exit/Escape/history evidence plus paired blocked/unblocked cube drag on5676bd0 with final57/57 and illustrated1237/master38. Native pan delivery remains unresolved; physical input is QA52. |
...
| QA-45 | Move/rotate/copy | Core — partial, not passed | Extensive paired exact-value/local-frame/Copy/Escape/history evidence plus clean 50/50 current-tree direct line/circle/rectangle and mixed line+circle Copy matrix. Fresh paired mixed-selection and compact-layout checks remain open. 2026-09-13: compact transform-control check closed under QA-53 (compact suite 5/5); native mixed selection reached by marquee, typed move paired with the clone (evidence local). Published in batch 3 (2026-09-13); Rotate Around Axis default recorded. |
...
| QA-53 | Layout | Core — partial, not passed | Panel/palette correction4b6a78f passed final50/50 and changed live both-panel/inward-flyout/plane-choice checks; illustrated1247/master38 published89dbafc. Retained51/51 layout baseline remains valid. Native Mac lacks the same handedness/large-text controls; physical iPad variants remain unverified. |
...
September 12: typed planar-face/body/profile chooser on ab6fb2d passes26/26.
Changed live candidate dispatch, Cancel and gallery reopen preserve three bodies
and the new circle. Native Profile identity is confirmed by accessibility selection
of Sketch05's pentagon; earlier coordinate-only attempt selected Sketch12 and is
excluded. Later paired constraint receipts cover ordered additive sketch selection;
retained connected rectangle double-click remains valid. Curved-face/edge overlap
probed natively 2026-09-13 (a cylinder: wall → face with radius and Offset Face; rim → edge with
length/radius and Chamfer/Fillet; Select Through on the wall → both wall faces, body, profile) and
matched at 637a24d; see testing/sketch-parity-curved-edge-selection-2026-09-13.md.
Remaining: native's edge-only selection state, circular-edge radius readout.
...
Dense values, overlaps, manual reposition if supported, camera zoom. Issues: DM-06/07. Result: paired 2026-09-13 (layout/qa29-dense-zoom-live-2026-09-13): dense values read in full with grouped thousands after the clone's label-format fix (f27e277); manual reposition — corrected with screen-coordinate drags: native moves a linear label's leader, selection-lived for a measured label, saved and undoable for a driving one; the clone had this only for diameter labels and gained it for linear labels at 97f320a; camera zoom — labels keep screen size on both, native unclamped vs clone canvas-clamped (recorded). See testing/sketch-parity-dense-zoom-2026-09-13.md. Owner: Claude Code session.
