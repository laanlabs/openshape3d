...
| QA-29 | Badge layout | Core — partial live pass | Portrait and landscape edge keypad usable; resize alignment fixed; right-palette badge and compact keypad overlaps fixed 2026-09-13 (273f0c4); the three logged DimensionUITests failures were grid snapping, tests green (9e4b2cc). 2026-09-13 dense/reposition/zoom paired: dense labels read in full (clone `%g` cap fixed, f27e277), linear labels have no drag on either side, labels keep screen size across zoom (native unclamped, clone clamped to canvas). Remaining: native circle-label reposition observation. |
...
Dense values, overlaps, manual reposition if supported, camera zoom. Issues: DM-06/07. Result: paired 2026-09-13 (layout/qa29-dense-zoom-live-2026-09-13): dense values read in full with grouped thousands after the clone's label-format fix (f27e277); manual reposition — native linear labels have none (a drag deselects), clone matches, circle-label drag unobserved; camera zoom — labels keep screen size on both, native unclamped vs clone canvas-clamped (recorded). See testing/sketch-parity-dense-zoom-2026-09-13.md. Owner: Claude Code session.
