...
- **Branch-wide regression before merge:** full serial suite at dbbbc0a = unit 1611/1611 (1 skipped), UI 171/185 with ten failures; all ten attributed and fixed in targeted gates (fa7f22b defaults reset ×6, b4a1bb0 cylinder edge routing, ad135ba symmetry rail tests ×2, 05bbf71 mesh-unit prompt). The full suite is **re-running at the final state** (/tmp/os3d-full-suite2-20260913.xcresult, started 20:40, unit 1612 executed / 0 failures already in); its result goes into testing/sketch-parity-branch-regression-2026-09-13.md before merge.
...
- **Next:** record the full-suite result; then a physical iPad session for QA-52 (needs Jason). PR #29 stays draft until the suite result is in and its description is refreshed (draft body prepared in the session scratchpad, not applied).
